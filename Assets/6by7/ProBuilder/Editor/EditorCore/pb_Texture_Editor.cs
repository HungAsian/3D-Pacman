...
#region INITIALIZATION / EXIT HANDLING
	...
	public static pb_Texture_Editor Init(pb_Editor obj_editor)
	{
		pb_Texture_Editor win = (pb_Texture_Editor)EditorWindow.GetWindow(typeof(pb_Texture_Editor), true, "Texture Editor", true);
		win.editor = obj_editor;
		win.OnInitialize();
		return win;
	}
	...
	{
		editor.drawHandles = (EditorPrefs.HasKey(pb_Constant.pbDefaultHideFaceMask)) ? 
	...
	public void SetSelectionMode(SelectMode selectMode)
	{
		if(editor)
			editor.SetSelectionMode(selectMode);
	}
...
#region ONGUI
	...
	void OnGUI()
	{
		...
		if(GUI.Button(applyButtonRect, "Apply", EditorStyles.miniButton))
			ApplyMaterial(editor.selection, queuedMat);
		...
		GUILayout.BeginHorizontal();
			...
			if(GUI.changed) { SetFlipU(uv_gui.flipU, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
			...
			if(GUI.changed) { SetFlipV(uv_gui.flipV, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
			...
			if(GUI.changed) { SetSwapUV(uv_gui.swapUV, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
		...
		GUILayout.BeginHorizontal();
			...
			GUI.changed = false;
			editor.drawHandles = GUILayout.Toggle(editor.drawHandles, "Highlight", GUILayout.MaxWidth(oneThird), GUILayout.MinWidth(oneThird));
			...
			if(GUI.changed) { SetUseWorldSpace(uv_gui.useWorldSpace, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
		...
		GUILayout.BeginVertical();
		{
			...
			GUILayout.BeginHorizontal();
				...
				if(GUI.changed) { SetScale(uv_gui.scale, XY.X, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
				...
				if(GUI.changed) { SetScale(uv_gui.scale, XY.Y, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
			GUILayout.EndHorizontal();
			...
			GUILayout.BeginHorizontal();
				...
				if(GUI.changed) { SetOffset(uv_gui.offset, XY.X, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
				...
				if(GUI.changed) { SetOffset(uv_gui.offset, XY.Y, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
			GUILayout.EndHorizontal();
			...
			GUILayout.BeginHorizontal();
				...
				if(GUI.changed) { SetRotation(uv_gui.rotation, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
		...
		// HAX
		pb_Object[] sel = editor.selection;
		...
		GUILayout.BeginVertical();
			...
			GUILayout.BeginHorizontal();
				...
				if(GUI.changed) { SetFill(uv_gui.fill, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
			GUILayout.EndHorizontal();	
		#if TORNADO_TWINS
			GUI.enabled = false;
		#endif
			...
			GUILayout.BeginHorizontal();
				...
				if(GUI.changed) { SetJustify(uv_gui.justify, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
			GUILayout.EndHorizontal();
			...
			GUILayout.BeginHorizontal();
				...
				if(GUI.changed) { SetProjectionAxis(uv_gui.projectionAxis, editor.selection);  UpdateDiffDictionary();  GUI.changed = false; }
			GUILayout.EndHorizontal();
		GUILayout.EndVertical();
...
#region MODIFY SINGLE PROPERTIES
	 ...
	 *	a change.
	 */
	public void TranslateOffset(Vector2 delta, pb_Object[] sel)
	{
		delta.x = -delta.x;
		for(int i = 0; i < sel.Length; i++)
		{
	...
	public void TranslateRotation(float delta, pb_Object[] sel)
	{
		#if !FREE
		for(int i = 0; i < sel.Length; i++)
		{
	...
	public void TranslateScale(Vector2 delta, pb_Object[] sel)
	{
		#if !FREE
		for(int i = 0; i < sel.Length; i++)
		{
	...
	// These could be public if we added a editor.selection param
...
#region MODIFY PER-UV SETTINGS
	...
	public void ApplyMaterial(pb_Object[] selected, Material mat)
	{
		...
		{
			ProBuilder.EntityType ent = pb.GetComponent<pb_Entity>().entityType;
			if(ent == ProBuilder.EntityType.Trigger || ent  == ProBuilder.EntityType.Collider)
				continue;
		...
		foreach(pb_Object pb in selected)
		{
			if(pb.GetComponent<pb_Entity>().entityType == ProBuilder.EntityType.Trigger || 
				pb.GetComponent<pb_Entity>().entityType == ProBuilder.EntityType.Collider)
				continue;
...
#region EDITOR CACHES
	...
	public void OnSelectionUpdate(pb_Object[] sel)
	{
		...
		if(editor.selection != null && editor.selection.Length > 0) 
		{
			if(editor.selection[0].SelectedFaces.Length > 0) {
				uv_gui = editor.selection[0].SelectedFaces[0].uv;
				
				currentMat = editor.selection[0].SelectedFaces[0].material;
		...
		if(smoothingEditor) smoothingEditor.UpdateSelection(editor.selection);
	}
...
#region WINDOW MANAGEMENT
	...
	{
		smoothingEditor = pb_Smoothing_Editor.Init(this, editor.selection);
	}
#endregion
