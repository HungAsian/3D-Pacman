...
#region CONSTANT & GUI MEMEBRS
	...
	#else
	static Vector2 FLOATING_SCREEN_SIZE = new Vector2(52, 594);	// 674
	static Vector2 DOCKED_SCREEN_SIZE = new Vector2(52, 592);	// 673
	#endif
...
#region INITIALIZATION AND ONDISABLE
	...
	public void OnEnable()
	{
		...
		ToggleEntityVisibility(ProBuilder.EntityType.Detail, true);
	...
	public void LoadPrefs()
	{
		...
		if(!EditorPrefs.HasKey(pb_Constant.pbEditorPrefVersion) || EditorPrefs.GetInt(pb_Constant.pbEditorPrefVersion) < EDITOR_PREF_VERSION ) {
			...
			Debug.LogWarning("ProBuilder: Clearing shortcuts. There were some internal changes in 2.2.1b5 that required we rebuild this cache.  This will only happen once, and everything else is okay.\n\nWell, except whatever custom shortcut keys you set.  Those are nuked.");
		}

		editLevel 			= pb_Preferences_Internal.GetEnum<EditLevel>(pb_Constant.pbDefaultEditLevel);

		selectionMode 		= pbUtil.ParseEnum(EditorPrefs.GetString(pb_Constant.pbDefaultSelectionMode), selectionMode);
		shortcuts 			= pb_Shortcut.ParseShortcuts(EditorPrefs.GetString(pb_Constant.pbDefaultShortcuts));
		handleAlignment 	= pbUtil.ParseEnum(EditorPrefs.GetString(pb_Constant.pbHandleAlignment), handleAlignment);
		limitFaceDragCheckToSelection = EditorPrefs.HasKey(pb_Constant.pbDragCheckLimit) ? EditorPrefs.GetBool(pb_Constant.pbDragCheckLimit) : true;
	}
...
#region ONGUI
	...
	void OnGUI()
	{
		if(!guiInitialized) 
		{
			...
			pbStyle.padding = new RectOffset(2,2,1,0);
		...
		if(GUILayout.Button(dgraphic, pbStyle, GUILayout.MaxWidth(BUTTON_WIDTH)))
		{
			...
			ToggleEntityVisibility(ProBuilder.EntityType.Detail, show_Detail);
		...
		if(GUILayout.Button(wgraphic, pbStyle, GUILayout.MaxWidth(BUTTON_WIDTH)))
		{
			...
			ToggleEntityVisibility(ProBuilder.EntityType.Occluder, show_Occluder);
		}
		#endif
		...
		if(GUILayout.Button(mgraphic, pbStyle, GUILayout.MaxWidth(BUTTON_WIDTH)))
		{
			...
			ToggleEntityVisibility(ProBuilder.EntityType.Mover, show_Mover);
		}
		...
		if(GUILayout.Button(cgraphic, pbStyle, GUILayout.MaxWidth(BUTTON_WIDTH)))
		{
			...
			ToggleEntityVisibility(ProBuilder.EntityType.Collider, show_Collider);
		}
		...
		if(GUILayout.Button(tgraphic, pbStyle, GUILayout.MaxWidth(BUTTON_WIDTH)))
		{
			...
			ToggleEntityVisibility(ProBuilder.EntityType.Trigger, show_Trigger);
		}
...
#region RAYCASTING AND DRAGGING
	...
	public const float MAX_EDGE_SELECT_DISTANCE = 10f;
	...
	private pb_Object RaycastCheck(Vector3 mousePosition)
	{
		...
		else
		{
			...
			pb_Face selectedQuad = pb.FaceWithTriangle(tri);
			if(selectedQuad != null)
			{
				...
				int indx = System.Array.IndexOf(pb.SelectedFaces, selectedQuad);
				if( indx > -1 ) {
					pb.RemoveFromFaceSelectionAtIndex(indx);
				} else {
					pb.AddToFaceSelection(selectedQuad);
				}
			}
		}
	...
	private void DragCheck()
	{
		switch(selectionMode)
		{
			...
			case SelectMode.Face:
			{
				...
				List<pb_Face> selectedFaces = new List<pb_Face>();
...
#region VERTEX TOOLS
				...
				for(int n = 0; n < selection[i].SelectedTriangles.Length; n++)
				{
					switch(handleAlignment)
					{
						case HandleAlignment.Plane:
							...
							Vector3 nrm = pb_Math.PlaneNormal(vertexOrigins[i]);
	...
	private void VertexRotateTool()
	{
		...
		if(currentHandleRotation != previousHandleRotation)
		{
			...
			for(int i = 0; i < selection.Length; i++)
			{
				Vector3 nrm = (vertexOrigins[i].Length > 2) ? pb_Math.PlaneNormal(vertexOrigins[i]) : Vector3.zero;
	...
	public void TextureScaleTool()
	{
		...
		if(t_texScale != texScale)
		{
			...
			#endif
			textureWindow.TranslateScale(delta* scaleModifier, selection);
		}
	}
	#endif
#endregion
	...
	private bool TopLevelShortcuts(pb_Shortcut shortcut)
	{
		...
		switch(shortcut.action)
		{
			...
			case "Set Trigger":
				...
				foreach(pb_Object pb in selection)
					pb_Editor_Utility.SetEntityType(ProBuilder.EntityType.Trigger, pb.gameObject);
				break;
			...
			case "Set Occluder":
				if(editLevel == EditLevel.Top)
				foreach(pb_Object pb in selection)
					pb_Editor_Utility.SetEntityType(ProBuilder.EntityType.Occluder, pb.gameObject);
				break;
			#endif
			...
			case "Set Collider":
				if(editLevel == EditLevel.Top)
				foreach(pb_Object pb in selection)
					pb_Editor_Utility.SetEntityType(ProBuilder.EntityType.Collider, pb.gameObject);
				break;

			case "Set Mover":
				if(editLevel == EditLevel.Top)
				foreach(pb_Object pb in selection)
					pb_Editor_Utility.SetEntityType(ProBuilder.EntityType.Mover, pb.gameObject);
				break;
...
#region VIS GROUPS
	...
	public void ToggleEntityVisibility(ProBuilder.EntityType entityType, bool isVisible)
	{
...
#region TOOL SETTINGS
	...
	public void SetHandleAlignment(HandleAlignment ha)
	{
		...
		handleAlignment = ha;
		EditorPrefs.SetString(pb_Constant.pbHandleAlignment, handleAlignment.ToString());
	...
	{
		// if attempting to set to vertex mode and texture window is open, put a stop to that shit
		if(mode == SelectMode.Vertex && editLevel == EditLevel.Texture)
		...
		EditorPrefs.SetString(pb_Constant.pbDefaultSelectionMode, selectionMode.ToString());
	}
	...
	public void SetEditLevel(EditLevel el, bool forceCloseTextureWindow)
	{	
		switch(el)
		{
			case EditLevel.Top:
		...
		EditorPrefs.SetString(pb_Constant.pbDefaultEditLevel, editLevel.ToString());
	}
#endregion
...
#region SELECTION CACHING AND MANAGING
	...
	public void UpdateSelection()
	{
		...
		{			
			...
			selected_uniqueEdges_all[i] = pb_Edge.GetUniversalEdges(pb_Edge.AllEdges(pb.faces), pb.sharedIndices).Distinct().ToArray();
...
#region HANDLE AND GUI CALCULTATIONS
	...
	private void UpdateTextureHandles()
	{
		...
		Vector3 nrm = selected_verticesLocal_sel[0].Length > 2 ? pb_Math.PlaneNormal(selected_verticesLocal_sel[0]) : Vector3.up;
	...
	public void UpdateHandleRotation()
	{
		...
		foreach(Vector3[] v in selected_verticesLocal_sel)
			if(v.Length > 2)
			{
				selectedNormal_local = pb_Math.PlaneNormal(v);
				break;			
			}
...
#region EVENTS AND LISTENERS
	...
	public void OnHierarchyChange()
	{
		...
		if(!EditorApplication.isPlaying && !movingTextures && !movingVertices)
		{
			// don't delete, dummy!
			SceneWideDuplicateCheck();
		}
	...
	private void OnGeometryChanged( pb_Object[] pbs )
	{
		...
		foreach(pb_Object pb in pbs)
		{
			...
			else
			{
				...
				if(pb.GetComponent<pb_Entity>().entityType != ProBuilder.EntityType.Mover)
				{
...
#region WINDOW MANAGEMENT
	...
	#if !PROTOTYPE
	public void OpenTextureWindow(pb_Texture_Editor existingWindow)
	{
		prevSelectMode = selectionMode;
		prevEditMode = editLevel;
		
		SetSelectionMode(SelectMode.Face);
		textureWindow = existingWindow;
		UpdateTextureHandles();
	}
	...
	public void OpenTextureWindow()
	{
		...
		SetSelectionMode(SelectMode.Face);
		textureWindow = pb_Texture_Editor.Init(this);
		UpdateTextureHandles();
	}

	SelectMode prevSelectMode;
	EditLevel prevEditMode;
	public void CloseTextureEditor(bool callingFromTextureWindow)
	{
		...
		if(!callingFromTextureWindow && textureWindow) {
			...
			SetEditLevel(prevEditMode, false);
		}

		textureWindow = null;

		if(callingFromTextureWindow)	
			SetSelectionMode(prevSelectMode);

		drawHandles = true;

		// if(callingFromTextureWindow) {
		// 	Debug.Log("Setting edit level top");
		// 	SetEditLevel(EditLevel.Top);
		// }
	}
	#endif
	...
	public void DrawFaceNormals()
	{
		foreach(pb_Object pb in selection)
		{
			...
			for(int i = 0; i < faces.Length; i++)
			{
				...
				Vector3 v = pb.transform.TransformPoint(pb.FaceCenter(faces[i]));
				Vector3 nrml = pb.FaceNormal(faces[i]);

				Vector3 vup = v;
				pb.transform.localRotation.ToAngleAxis(out angle, out vup);

				Handles.color = Color.green;
					Handles.DrawLine(v, v + Quaternion.AngleAxis(angle, vup) * nrml);
				Handles.color = Color.white;
			}
		}
	}
