...
public class pb_Shape
{
	...
	{
		Vector3[] points = new Vector3[pb_Object.TRIANGLES_CUBE.Length];
		for(int i = 0; i < pb_Object.TRIANGLES_CUBE.Length; i++)
			points[i] = Vector3.Scale(pb_Object.VERTICES_CUBE[pb_Object.TRIANGLES_CUBE[i]], size);
	...
	/**
	 *	\brief Returns a pb_Object door with the passed size.
	 * @param size Size to apply to generated object.
	 * \returns New #pb_Object.
	 */
	public static pb_Object DoorGenerator(Vector3 size)
	{
		Vector3[] template = new Vector3[12]
		{
			// _ _ _ _
			Vector3.Scale(new Vector3(-2f, 0f, .5f), size),	// 0
			Vector3.Scale(new Vector3(-1f, 0f, .5f), size),	// 1
			Vector3.Scale(new Vector3(1f, 0f, .5f), size),	// 2
			Vector3.Scale(new Vector3(2f, 0f, .5f), size),	// 3
			// . . . .
			// _ _ _ _ 
			Vector3.Scale(new Vector3(-2f, 3f, .5f), size),	// 4
			Vector3.Scale(new Vector3(-1f, 3f, .5f), size),	// 5
			Vector3.Scale(new Vector3(1f, 3f, .5f), size),	// 6
			Vector3.Scale(new Vector3(2f, 3f, .5f), size),	// 7
			// - - - - 
			// . . . . 
			// _ _ _ _
			Vector3.Scale(new Vector3(-2f, 4f, .5f), size),	// 8
			Vector3.Scale(new Vector3(-1f, 4f, .5f), size),	// 9
			Vector3.Scale(new Vector3(1f, 4f, .5f), size),	// 10
			Vector3.Scale(new Vector3(2f, 4f, .5f), size)	// 11
		};
	 ...
	 * \returns New #pb_Object.
	 */
	public static pb_Object PlaneGenerator(float _width, float _height, int widthCuts, int heightCuts, ProBuilder.Axis axis, bool smooth)
	{
		float width = _width;
		...
		switch(axis)
		{
			case ProBuilder.Axis.Right:
				for(i = 0; i < v.Length; i++)
					v[i] = new Vector3(0f, p[i].x, p[i].y);
				break;
			case ProBuilder.Axis.Left:
				for(i = 0; i < v.Length; i++)
					v[i] = new Vector3(0f, p[i].y, p[i].x);
				break;
			case ProBuilder.Axis.Up:
				for(i = 0; i < v.Length; i++)
					v[i] = new Vector3(p[i].y, 0f, p[i].x);
				break;
			case ProBuilder.Axis.Down:
				for(i = 0; i < v.Length; i++)
					v[i] = new Vector3(p[i].x, 0f, p[i].y);
				break;				
			case ProBuilder.Axis.Forward:
				for(i = 0; i < v.Length; i++)
					v[i] = new Vector3(p[i].x, p[i].y, 0f);
				break;
			case ProBuilder.Axis.Backward:
				for(i = 0; i < v.Length; i++)
