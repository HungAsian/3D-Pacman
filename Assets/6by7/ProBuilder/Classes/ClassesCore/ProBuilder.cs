...
#region PUBLIC_METHODS
	...
	public static pb_Object CreatePrimitive(Shape shape)
	{
		...
		{
			case Shape.Cube:
				pb = pb_Shape.CubeGenerator(Vector3.one);
				break;

			case Shape.Prism:
				pb = pb_Shape.PrismGenerator(Vector3.one);
				break;

			case Shape.Stair:
				// steps, width, height, depth, extend sides to floor, generate back, platforms only
				pb = pb_Shape.StairGenerator(5, 2f, 5f, 7f, true, true, false);
				break;

			case Shape.Cylinder:
				pb = pb_Shape.CylinderGenerator(6, 1.5f, 4f, 2);
				break;

			case Shape.Plane:
				pb = pb_Shape.PlaneGenerator(2f, 2f, 3, 3, ProBuilder.Axis.Up, false);
				break;
