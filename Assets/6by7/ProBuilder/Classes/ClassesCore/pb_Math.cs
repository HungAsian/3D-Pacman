...
namespace ProBuilder2.Math
{
	...
	public static class pb_Math
	{
		...
		public static Vector3 PlaneNormal(Vector3 p0, Vector3 p1, Vector3 p2)
		{
		...
		public static Vector3 PlaneNormal(Vector3[] p)
		{
		...
		public static Vector3 PlaneNormal(List<Vector3> p)
		{
			Vector3 cross = Vector3.Cross(p[1] - p[0], p[2] - p[0]);
			if (cross.magnitude < Mathf.Epsilon)
				return new Vector3(0f, 0f, 0f); // bad triangle
		...
		public static Vector2 RotateAroundPoint(this Vector2 v, Vector2 origin, float thetaDeg)
		{
			float cx = origin.x, cy = origin.y;	// origin
			float px = v.x, py = v.y;			// point

			float s = Mathf.Sin(thetaDeg * Mathf.Deg2Rad);
			float c = Mathf.Cos(thetaDeg * Mathf.Deg2Rad);
		 ...
		 */
		internal static pb_UV.ProjectionAxis GetProjectionAxis(Vector3 plane)
		{
